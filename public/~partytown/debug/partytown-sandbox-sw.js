...
((window) => {
                      ...
                      : cstrName.endsWith("Event")
                        ...
                        : "CSSRuleList" === cstrName
                          ...
                          : startsWith(cstrName, "CSS") &&
                            ...
                            ? [
                                SerializedType.CSSRule,
                                serializeCssRuleForWorker(value),
                              ]
  ...
  const readNextScript = (worker, winCtx) => {
    ...
    const blockingScriptSelector = `${scriptSelector}:not([async]):not([defer])`;
    let scriptElm;
    let $instanceId$;
    let scriptData;
    if (doc?.body) {
  ...
  const readMainInterfaces = () => {
    const elms = Object.getOwnPropertyNames(mainWindow)
      ...
      .filter((elm) => {
        ...
        return !(
          "HTMLUnknownElement" === constructorName &&
          "UNKNOWN" !== elm.nodeName.toUpperCase()
        );
      })
      .map((elm) => [elm]);
  ...
  ((receiveMessage) => {
    ...
    return swContainer.getRegistration().then((swRegistration) => {
      ...
      return (worker, msg) => {
        msg[0] === WorkerMessageType.MainDataRequestFromWorker
          ? worker.postMessage([
