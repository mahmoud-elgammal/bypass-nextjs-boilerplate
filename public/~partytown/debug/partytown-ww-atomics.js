...
((self) => {
  ...
  !((WorkerMessageType) => {
    ...
    WorkerMessageType[(WorkerMessageType.InitializedWebWorker = 4)] =
      "InitializedWebWorker";
    WorkerMessageType[(WorkerMessageType.InitializeEnvironment = 5)] =
      "InitializeEnvironment";
    WorkerMessageType[(WorkerMessageType.InitializedEnvironmentScript = 6)] =
      "InitializedEnvironmentScript";
    WorkerMessageType[(WorkerMessageType.InitializeNextScript = 7)] =
      "InitializeNextScript";
    WorkerMessageType[(WorkerMessageType.InitializedScripts = 8)] =
      "InitializedScripts";
    WorkerMessageType[(WorkerMessageType.RefHandlerCallback = 9)] =
      "RefHandlerCallback";
    WorkerMessageType[(WorkerMessageType.ForwardMainTrigger = 10)] =
      "ForwardMainTrigger";
    WorkerMessageType[(WorkerMessageType.ForwardWorkerAccessRequest = 11)] =
      "ForwardWorkerAccessRequest";
    WorkerMessageType[(WorkerMessageType.AsyncAccessRequest = 12)] =
      "AsyncAccessRequest";
    WorkerMessageType[(WorkerMessageType.LocationUpdate = 13)] =
      "LocationUpdate";
    WorkerMessageType[(WorkerMessageType.DocumentVisibilityState = 14)] =
      "DocumentVisibilityState";
    WorkerMessageType[(WorkerMessageType.CustomElementCallback = 15)] =
  ...
  !((InterfaceType) => {
    ...
    InterfaceType[(InterfaceType.ProcessingInstructionNode = 7)] =
    ...
    InterfaceType[(InterfaceType.DocumentFragmentNode = 11)] =
      "DocumentFragmentNode";
    InterfaceType[(InterfaceType.EnvGlobalConstructor = 12)] =
      "EnvGlobalConstructor";
  ...
  !((SerializedType) => {
    ...
    SerializedType[(SerializedType.CSSStyleDeclaration = 13)] =
      "CSSStyleDeclaration";
  ...
  !((CallType) => {
    ...
    CallType[(CallType.NonBlockingNoSideEffect = 3)] =
      "NonBlockingNoSideEffect";
  })(CallType || (CallType = {}));
  ...
  const getInstanceCacheKey = (instance, memberName, args) =>
    [
      ...
      ...(args || EMPTY_ARRAY).map((arg) =>
        String(arg?.[WinIdKey] ? arg[InstanceIdKey] : arg),
      ),
    ].join(".");
            ...
            : "object" === type
              ? value[InstanceIdKey]
                ? [
                    ...
                    [value[WinIdKey], value[InstanceIdKey]],
                  ]
                  ...
                  : supportsTrustedHTML && value instanceof TrustedHTML
                    ...
                    : value instanceof ArrayBuffer
                      ...
                      : ArrayBuffer.isView(value)
                        ? [
                            SerializedType.ArrayBufferView,
                            value.buffer,
    ...
    if (serializedValueTransfer) {
      ...
      if (serializedType === SerializedType.Event) {
        if ("message" === obj.type && obj.origin) {
          ...
          const postMessageData = postMessages.find(
  ...
  const deserializeRefFromMain = (
    applyPath,
    { $winId$, $instanceId$, $nodeName$, $refId$ },
  ) => {
    webWorkerRefsByRefId[$refId$] ||
      webWorkerRefIdsByRef.set(
        (webWorkerRefsByRefId[$refId$] = (...args) => {
  ...
  const getTargetProp = (target, applyPath) => {
    ...
    if (target) {
      ...
      if ("Window" === cstrName) {
        ...
        n =
          "nodeType" in target &&
          target.nodeType === InterfaceType.AttributeNode
            ? "attributes."
    ...
    if (len(taskQueue)) {
      ...
      if (isBlocking) {
        const accessRsp = ((webWorkerCtx, accessReq) => {
          ...
          webWorkerCtx.$postMessage$([
            WorkerMessageType.ForwardWorkerAccessRequest,
            accessReq,
          ]);
  ...
  const setter = (instance, applyPath, value, hookSetterValue) => {
    ...
    applyPath = [
      ...applyPath,
      serializeInstanceForMain(instance, value),
  ...
  const callMethod = (
    ...
    callType =
      callType ||
      (nonBlockingMethods.includes(methodName)
  ...
  const run = (env, scriptContent, scriptUrl) => {
    ...
    const sourceWithReplacedThis = ((scriptContent, newThis) =>
  ...
  const HTMLSrcElementDescriptorMap = {
    ...
    onload: {
      get() {
        const callbacks = getInstanceStateValue(this, StateProp.loadHandlers);
    ...
    onerror: {
      get() {
        const callbacks = getInstanceStateValue(this, StateProp.errorHandlers);
        return callbacks?.[0] || null;
      },
      set(cb) {
    ...
    const WorkerNode = defineConstructorName(
      class extends WorkerBase {
        ...
        insertBefore(newNode, referenceNode) {
          ...
          if (isScript) {
            const scriptContent = getInstanceStateValue(
              newNode,
              StateProp.innerHTML,
            );
            ...
            if (scriptContent) {
              if (isScriptJsType(scriptType)) {
                ...
                const loadOnMainThread =
                  scriptId &&
                  testIfMustLoadScriptOnMainThread(config, scriptId);
                if (loadOnMainThread) {
            ...
            ((winId, iframe) => {
              ...
              const callback = () => {
                if (
                  ...
                  type = getInstanceStateValue(
                    iframe,
                    StateProp.loadErrorStatus,
                  )
                  ...
                  handlers = getInstanceStateValue(
                    iframe,
                    StateProp.errorHandlers,
                  );
                  handlers?.map((handler) =>
          ...
          if (isScript) {
            sendToMain(true);
            webWorkerCtx.$postMessage$([
              WorkerMessageType.InitializeNextScript,
              winId,
            ]);
          }
          return newNode;
    ...
    const DocumentDescriptorMap = {
      ...
      createTextNode: {
        value(text) {
          const winId = this[WinIdKey];
          const instanceId = randomId();
          const textNode = getOrCreateNodeInstance(
            winId,
            instanceId,
            NodeName.Text,
          );
  ...
  const patchHTMLIFrameElement = (WorkerHTMLIFrameElement, _env) => {
        ...
        set(src) {
          if (src) {
            if (src.startsWith("javascript:")) {
              ...
              env.$isLoading$ = 1;
              env.$isSameOrigin$ =
                webWorkerCtx.$origin$ === env.$location$.origin;
              ...
              if (xhrStatus > 199 && xhrStatus < 300) {
                setter(
                  ...
                  `<base href="${src}">` +
                    ((text) =>
                      text.replace(SCRIPT_TAG_REGEXP, (_, attrs) => {
                        const parts = [];
                        let hasType = false;
                        let match;
  ...
  const createWindow = (
    ...
    const WorkerBase = class {
      constructor(winId, instanceId, applyPath, instanceData, namespace) {
        ...
        this[InstanceStateKey] = cstrPrevInstance?.[InstanceStateKey] || {};
        ...
        constructor() {
              ...
              (() => {
                if (!webWorkerCtx.$initWindowMedia$) {
                  ...
                  webWorkerCtx.$importScripts$(
                    partytownLibUrl("partytown-media.js?v=0.11.2"),
                  );
          ...
          Object.assign(env, {
            ...
            $window$: new Proxy(win, {
              get: (win, propName) => {
                var _a;
                if ("string" !== typeof propName || Number.isNaN(propName)) {
                  return (
  ...
  const createEnvironment = (
    { $winId$, $parentWinId$, $url$, $visibilityState$ },
    isIframeWindow,
    isDocumentImplementation,
  ) => {
  ...
  const receiveMessageFromSandboxToWorker = (ev) => {
    ...
    if (webWorkerCtx.$isInitialized$) {
      ...
      webWorkerCtx.$postMessage$([
        WorkerMessageType.MainInterfacesRequestFromWorker,
      ]);
